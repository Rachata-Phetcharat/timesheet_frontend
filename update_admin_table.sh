#!/bin/bash
file="src/pages/AdminReportPage.tsx"
awk '
...
/}\)/ {
  ...
  if ($0 == "  })") {
    seen_filter = 1
    print "  const totalPages = Math.ceil(filteredAttendance.length / itemsPerPage)"
    print "  React.useEffect(() => {"
    print "    setCurrentPage(1)"
    print "  }, [searchTerm, statusFilter, selectedMonth])"
    print "  const paginatedAttendance = filteredAttendance.slice((currentPage - 1) * itemsPerPage, currentPage * itemsPerPage)"
  }
  next
}
/filteredAttendance\.length === 0/ {
  sub(/filteredAttendance\.length === 0/, "paginatedAttendance.length === 0")
  print $0
  next
}
/filteredAttendance\.map\(/ {
  sub(/filteredAttendance\.map/, "paginatedAttendance.map")
  print $0
  next
}
/<\/Table>/ {
